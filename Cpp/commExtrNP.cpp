...
extern "C" {
    ...
    double modularity(double **adj_matrix, int *config, int dim, double *param, vector<int> selected, vector<int> unselected) {
        double eval = 0;
        int sum = 0;
        for (int i = 0; i < dim; i++) {
            sum += config[i];
    ...
    double localMoving(double **adj_matrix, vector< vector<int> > const& adj_list, int *community, int dim, int verbose) {
    ...
    void commextr(int *d, int *ps, double *matrix, int *res, int *followup, int *vb)
    {
        int verbose = *vb;
        int DIM = *d;
        int POPSIZE = *ps;
...
//        }
        ...
        for (int i = 0; i < newPopsize; i++) {
            if (verbose) {
                cout<<"Now evolving individual no. "<<i+1<<"\n";
            }
...
//        }
        ...
        if (verbose) {
            cout<<"Search process done."<<endl;
            cout<<"Best fitness W is: "<<fitList[bestF]<<endl;
            cout<<"Best community size is: "<<commSize<<endl;
        }
        for (int i = 0; i != DIM; i++) {
            delete [] adj_matrix[i];
        }
